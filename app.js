...
function renderSubjects() {
  ...
  SUBJECTS.forEach(subject => {
    ...
    if (pct < 34) {
      val.style.stroke = rootStyle.getPropertyValue("--circle-red");
    } else if (pct < 67) {
      val.style.stroke = rootStyle.getPropertyValue("--circle-orange");
    } else {
      val.style.stroke = rootStyle.getPropertyValue("--circle-green");
    }
...
function updateStatsOverview() {
  ...
  if (subjEl) subjEl.textContent = `${masteredSubjects}/13`;
...
function loadEventDetails() {
  ...
  const ev = state.events[calendarSelectedISO];
  if (ev) {
...
function deleteEvent() {
  if (!calendarSelectedISO) return;
  if (state.events[calendarSelectedISO]) {
