...
function loadState() {
  ...
  // initialize each subject with real subtopics if missing or count mismatch
  SUBJECTS.forEach(s => {
    const names = SUBJECT_SUBTOPICS[s.code] || [];
    if (!state.subtopics[s.code] || state.subtopics[s.code].length !== names.length) {
...
function renderSubjects() {
  ...
  SUBJECTS.forEach(subject => {
    ...
    list.forEach((topic, idx) => {
      ...
      name.textContent = topic.name; // Show real label
...
function renderCalendar() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    dayEl.addEventListener("click", () => {
      ...
      renderCalendar();
      loadEventDetails();
      updateExamCountdown(); // update countdown immediately when selecting
    });
...
function updateExamCountdown() {
  const countdownEl = document.getElementById("next-exam-countdown");
  if (!countdownEl || !state.events) return;

  const today = new Date();
  const upcomingExams = Object.keys(state.events)
  ...
  const nextExamISO = upcomingExams[0];
  const nextExam = state.events[nextExamISO];
  const diffDays = Math.ceil((new Date(nextExamISO) - today) / (1000 * 60 * 60 * 24));

  countdownEl.textContent = `Next exam: ${nextExam.subjectCode} in ${diffDays} day${diffDays !== 1 ? "s" : ""}`;
}

// refresh countdown automatically every midnight
function scheduleCountdownRefresh() {
  ...
  setTimeout(() => {
    updateExamCountdown();
    scheduleCountdownRefresh(); // schedule next day
  }, msUntilMidnight);
}
